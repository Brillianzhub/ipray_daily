...
const useBookmarkManager = () => {
    ...
    const toggleSelection = (id) => {
        const newSelectedVerseIds = new Set(selectedVerseIds);
        if (newSelectedVerseIds.has(id)) {
            newSelectedVerseIds.delete(id);
        } else {
            newSelectedVerseIds.add(id);
        }
        setSelectedVerseIds(newSelectedVerseIds);
        setShowOptions(newSelectedVerseIds.size > 0);
